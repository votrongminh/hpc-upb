...
void StateManager::enterSolution(const State& res){
	...
	std::lock_guard<std::shared_timed_mutex> lock(bestSolMutex);
...
bool StateManager::claim(const State& val){
	...
	*/
	std::shared_lock<std::shared_timed_mutex> readlock(claimMutex);
	...
	readlock.unlock();
	std::lock_guard<std::shared_timed_mutex> lock(claimMutex);
	state_set.erase(val);
	state_set.insert(val);
