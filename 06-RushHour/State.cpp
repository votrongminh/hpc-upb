...
bool State::legal(const StateManager* pf) const {
	...
	if(steps > 500){printf("Maximum steps reached\n"); return false;}//Safety shutoff

	for(unsigned int i=0; i<cars.size(); ++i){
		if(cars[i].onField(pf->playfield.sizeX,pf->playfield.sizeY)==false){/*printf("Car %d would be off the field!\n",i);*/ return false;}
