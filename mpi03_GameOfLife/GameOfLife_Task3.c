...
void CalculateUpdate(int my_rank, int step, char* readFrom, char* writeTo, int sizeX, int totalSizeY){
	for(int y=totalSizeY-2;y>0;y--){
        for(int x=0;x<sizeX;x++){
			writeTo[x + sizeX*y] = readFrom[ x + sizeX*y ];
		}
	}

    int livingCellsCnt;
    for(int ourRow = 1; ourRow<totalSizeY-1;ourRow++) {
        for(int ourCol = 0; ourCol<sizeX;ourCol++) {
            livingCellsCnt = 0;
            /* For cells at column at most left [0] and right [sizeX-1], 
               Perform special rule due to lacking of neighbor*/
            if(ourCol==0) {
                for(int nborCol=0;nborCol<=1;nborCol++){
                    for(int nborRow=ourRow-1;nborRow<=ourRow+1;nborRow++){
                        if((nborCol!=ourCol)||(nborRow!=ourRow)){
                            if((int)readFrom[ nborCol + sizeX * nborRow ] !=0) {
                                livingCellsCnt++;
                            }
                        }
                    }
                }
            } else if (ourCol==sizeX-1) {
                for(int nborCol=ourCol-1;nborCol<=ourCol;nborCol++){
                    for(int nborRow=ourRow-1;nborRow<=ourRow+1;nborRow++){
                        if((nborCol!=ourCol)||(nborRow!=ourRow)){
                            if((int)readFrom[ nborCol + sizeX * nborRow ] !=0) {
                                livingCellsCnt++;
            ...
            } else { /* Otherwise, neighbor is considered as 8 cells sourounding */
                for(int nborCol=ourCol-1;nborCol<=ourCol+1;nborCol++){
                    for(int nborRow=ourRow-1;nborRow<=ourRow+1;nborRow++){
                        if((nborCol!=ourCol)||(nborRow!=ourRow)){
                            if((int)readFrom[ nborCol + sizeX * nborRow ] !=0) {
                                livingCellsCnt++;
            ...
            /* Rule1: A living cell stays alive if its surrounded by 2 or 3 living cells */
            if(((int)readFrom[ourCol+ourRow*sizeX] == 1)&&((livingCellsCnt==2)||(livingCellsCnt==3))){
                writeTo[ourCol+ourRow*sizeX] = 1;
            }
            
            /* Rule2:A dead cell comes alive if it's surrounded by exactly 3 live cells */
            if(((int)readFrom[ourCol+ourRow*sizeX]==0)&&(livingCellsCnt==3)){
                writeTo[ourCol+ourRow*sizeX] = 1;
            }
            
            /* Rule3: In all other cases the cell becaomes dead by over/underpopulation */
            if(livingCellsCnt < 2){
                writeTo[ourCol+ourRow*sizeX] = 0;
            }
            if(livingCellsCnt > 3){
                writeTo[ourCol+ourRow*sizeX] = 0;
            }
...
void ExchangeGhostlines(int my_rank,int total_ranks,int size, char* my_received_ghostline_top,char* my_received_ghostline_bot,char* pos_of_my_low,char* pos_of_my_high){
    ...
    if(my_rank==0) {
        MPI_Send(pos_of_my_low, size, MPI_CHAR, my_rank+1, 1, MPI_COMM_WORLD);
    ...
    if(my_rank==total_ranks -1) {
        MPI_Send(pos_of_my_high, size, MPI_CHAR, my_rank-1, 1, MPI_COMM_WORLD);
        for(int i=0;i<size;i++){
...
/* Exchange ghost line between ranks with Nonblocking
